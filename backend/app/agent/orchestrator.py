...
class AgentOrchestrator:
    ...
    async def process_request(self, audience: str, question: str, session_id: str = None, confirm: bool = False, tenant_id: str = None):
        ...
        try:
            ...
            from app.db.queries import get_db_connection, get_plan # Lazy import
            ...
            try:
                ...
                row = cur.fetchone()
                if row:
                    last_plan = get_plan(row[0]) # Returns dict
        ...
        if mode == "CHAT":
            ...
            if risk == "WRITE":
                if not confirm:
                    ...
                    create_action(action_id, session_id, tool_name, params_str, requires_confirmation=True)
                    
                    return {
                        "status": "needs_confirmation",
                        "message": f"I can {tool_def['description']}. Do you want to proceed?",
                        "proposed_action": {
                            "tool": tool_name,
                            "params": params,
                            "risk": risk
